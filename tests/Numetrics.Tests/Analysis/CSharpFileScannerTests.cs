...
public class CSharpFileScannerTests
{
    ...
    [Fact]
    public void AnalyzeSyntaxTrees_GlobalQualifiedUsingDirective_StripsGlobalPrefix()
    {
        // Auto-generated GlobalUsings.g.cs files use "global::" qualifier prefix
        const string code = """
        ...
        var trees = new[] { (CSharpSyntaxTree.ParseText(code), "TestAssembly") };
        var (_, globalUsings) = CSharpFileScanner.AnalyzeSyntaxTreesWithGlobalUsings(trees);

        globalUsings.ShouldContain("System");
        globalUsings.ShouldContain("System.Collections.Generic");
        globalUsings.ShouldNotContain("global::System");
        globalUsings.ShouldNotContain("global::System.Collections.Generic");
    }

    [Fact]
    public void ScanDirectory_GlobalUsingsGeneratedFile_IsIncludedInGlobalUsings()
    {
        var tempDir = Path.Combine(Path.GetTempPath(), Path.GetRandomFileName());
        var projectDir = Path.Combine(tempDir, "MyProject");
        ...
        try
        {
            ...
            File.WriteAllText(
                Path.Combine(projectDir, "MyProject.csproj"),
                "<Project Sdk=\"Microsoft.NET.Sdk\"><PropertyGroup><TargetFramework>net9.0</TargetFramework></PropertyGroup></Project>");
            const string globalUsingsContent = """
                // <auto-generated/>
                global using global::System;
                global using global::System.Collections.Generic;
                """;
            File.WriteAllText(
                Path.Combine(objDir, "MyProject.GlobalUsings.g.cs"),
                globalUsingsContent);

            var (_, globalUsings) = CSharpFileScanner.ScanDirectory(tempDir);
